#include "husky_dual_fr3_controller/robot_data.h"
...
namespace HuskyDualFR3Controller
{
    ...
    */
    RobotData::RobotData(const std::string& urdf_path, const bool verbose)
    {
        ...
        r_x_.setIdentity();
        r_xdot_.setZero();
        r_J_.setZero(6, model_.nv);
        r_Jdot_.setZero(6, model_.nv-3);
        l_x_.setIdentity();
        l_xdot_.setZero();
        l_J_.setZero(6, model_.nv);
        l_Jdot_.setZero(6, model_.nv-3);

        r_J_actuated_.setZero(6, model_.nv-3);
        l_J_actuated_.setZero(6, model_.nv-3);
    ...
    {        
        ...
        pinocchio::FrameIndex r_link_index = model_.getFrameId(r_ee_name_);
        if (r_link_index == static_cast<pinocchio::FrameIndex>(-1))
        {
            std::cerr << "\033[1;31m" << "Error: Link name " << r_ee_name_ << " not found in URDF." << "\033[0m" << std::endl;
            return false;
        }

        pinocchio::FrameIndex l_link_index = model_.getFrameId(l_ee_name_);
        if (l_link_index == static_cast<pinocchio::FrameIndex>(-1))
        {
            std::cerr << "\033[1;31m" << "Error: Link name " << l_ee_name_ << " not found in URDF." << "\033[0m" << std::endl;
            return false;
        }
        ...
        r_x_ = getPose(r_ee_name_);
        l_x_ = getPose(l_ee_name_);
        r_J_ = getJacobian(r_ee_name_);
        l_J_ = getJacobian(l_ee_name_);
        r_J_actuated_ = r_J_ * S_;
        l_J_actuated_ = l_J_ * S_;
        r_xdot_ = r_J_actuated_ * qdot.tail(model_.nv-3);
        l_xdot_ = l_J_actuated_ * qdot.tail(model_.nv-3);
        r_Jdot_ = getJacobianTimeVariation();
        l_Jdot_ = getJacobianTimeVariation();
