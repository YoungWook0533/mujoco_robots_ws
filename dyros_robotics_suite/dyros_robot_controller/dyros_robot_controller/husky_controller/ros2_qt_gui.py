...
class HuskyQtGui(Node, QWidget):
    def __init__(self):
        # ROS2 & Qt 초기화
        rclpy.init(args=None)
        Node.__init__(self, 'husky_controller_gui')
        QWidget.__init__(self)
        self.setWindowTitle('Husky Controller GUI')

        # Publishers
        self.mode_pub = self.create_publisher(Int32, 'husky_controller/mode_input', 10)
        self.pose_pub = self.create_publisher(Pose, 'husky_controller/target_pose', 10)
    ...
    def _on_publish_vel(self):
        msg = Twist()
        try:
            msg.linear.x = float(self.vel_inputs['v'].text())
