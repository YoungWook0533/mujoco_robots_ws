...
class PiperRobotData():
    def __init__(self, node: Node, mj_joint_names: list):
        ...
        self.ee_name = "link6"  # End-effector name in the URDF
    ...
    def updateState(self, 
        ...
        if(not self.updateKinematics(self.q, self.qdot)):
            self.node.get_logger().error("[PiperRobotData] Failed to update robot state.")
...
# ==================================================================================================
    def updateKinematics(self, q: np.ndarray, qdot: np.ndarray) -> bool:
        ...
        if q.size != self.nq or qdot.size != self.nv:
            self.node.get_logger().error(f"[PiperRobotData] Error: q size {q.size}, qdot size {qdot.size} do not match model dimensions.")
            return False
