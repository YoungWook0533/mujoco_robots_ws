...
class PCVQtGui(Node, QWidget):
    def __init__(self):
        # ROS2 & Qt 초기화
        rclpy.init(args=None)
        Node.__init__(self, 'pcv_controller_gui')
        QWidget.__init__(self)
        self.setWindowTitle('PCV Controller GUI')

        # Publishers
        self.mode_pub = self.create_publisher(Int32, 'pcv_controller/mode_input', 10)
        self.pose_pub = self.create_publisher(Pose, 'pcv_controller/target_pose', 10)
        self.vel_pub  = self.create_publisher(Twist, 'pcv_controller/cmd_vel', 10)

        # Subscribers
        self.joint_sub    = self.create_subscription(JointState, 'pcv_controller/joint_states', self.joint_state_callback, 10)
        self.base_pose_sub = self.create_subscription(Pose, 'pcv_controller/base_pose', self.base_pose_callback, 10)
        self.base_vel_sub  = self.create_subscription(Twist, 'pcv_controller/base_vel', self.base_vel_callback, 10)
