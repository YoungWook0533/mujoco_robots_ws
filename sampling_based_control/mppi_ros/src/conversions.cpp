...
#include <rclcpp/rclcpp.hpp> // For rclcpp::Clock
...
void to_msg(const mppi::Rollout& rollout, mppi_ros::msg::Rollout& rollout_ros, const bool input_only, const size_t max_length) {
  ...
  mppi_ros::msg::Array state, input, noise;
...
void to_msg(const mppi::data_t& data, mppi_ros::msg::Data& data_ros) {
  // Assign current time using ROS 2 builtin_interfaces/Time
  data_ros.time = rclcpp::Clock().now();
  ...
  //  mppi_ros::msg::Rollout rollout_ros;
