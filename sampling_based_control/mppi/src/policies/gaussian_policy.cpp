...
Eigen::VectorXd GaussianPolicy::nominal(double t) {
  // Find index to first element greater than t
  size_t upper_idx = std::distance(
      t_.data(), std::upper_bound(t_.data(), t_.data() + t_.size(), t));

  // If t is before the first timestamp, return the first nominal input
  if (upper_idx == 0) {
    return nominal_.row(0);
  }

  // If t is at or beyond the last timestamp, return the last nominal input
  if (upper_idx >= static_cast<size_t>(t_.size())) {
    return nominal_.row(t_.size() - 1);
  }

  // Otherwise, interpolate between the two surrounding samples
  size_t time_idx = upper_idx - 1;
...
Eigen::VectorXd GaussianPolicy::sample(double t, int k) {
  ...
  // Clamp to valid range [0, t_.size()-1]
  if (upper_idx == 0) {
    return samples_[k].row(0) + nominal_.row(0);
  }
  if (upper_idx >= static_cast<size_t>(t_.size())) {
    return samples_[k].row(t_.size() - 1) + nominal_.row(t_.size() - 1);
  }
